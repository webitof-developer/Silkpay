...
export default function DashboardPage() {
  ...
  const handleSyncBalance = async () => {
    ...
    try {
      ...
      const [dashboardRes] = await Promise.all([getDashboardOverview()]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <Button asChild className="w-full sm:w-auto">
            <Link href="/payouts/new">Create Payout</Link>
          </Button>
          <Button
            variant="outline"
              ...
              {loading ? (
                ...
                recentActivity.map((item, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-sm font-bold ${item.type === "REFUND" || item.type === "ADJUSTMENT" ? "text-green-600" : "text-red-600"}`}
                    >
                      {item.type === "REFUND" || item.type === "ADJUSTMENT"
                        ? "+"
                        : "-"}
